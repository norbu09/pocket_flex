defmodule PocketFlex.Recovery do
  ...
  defp do_retry_with_backoff(_error, _context, state, _retry_fn, _max_retries, current_retry, _base_delay, _max_delay) 
       when current_retry < 0 do
    {:ok, state}
  end
  
  defp do_retry_with_backoff(_error, _context, state, nil, _max_retries, _current_retry, _base_delay, _max_delay) do
    {:ok, state}
  end
  
  defp do_retry_with_backoff(_error, _context, state, _retry_fn, max_retries, current_retry, _base_delay, _max_delay) 
  ...
  defp do_retry_with_backoff(error, context, state, retry_fn, max_retries, current_retry, base_delay, max_delay) do
    ...
    try do
      case retry_fn.(state) do
        ...
        {:error, new_error} -> 
          Logger.warning("Retry attempt #{current_retry + 1} failed for error #{inspect(error)}: #{inspect(new_error)}")
      ...
      e -> 
        Logger.warning("Retry attempt #{current_retry + 1} failed for error #{inspect(error)}: #{inspect(e)}")
